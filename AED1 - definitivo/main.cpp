...
public:
    ...
    No() : funcao(-1), linha(-1), prox(nullptr) { }

    No(int _funcao, int _linha) : funcao(_funcao), linha(_linha), prox(nullptr) { }
};
...
public:
    ...
    void empilhaFuncao(int index, int linha) {
        No* aux = new No(index, linha);
        aux->prox = topo;
        topo = aux;
    }
